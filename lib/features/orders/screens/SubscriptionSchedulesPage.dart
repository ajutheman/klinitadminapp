...
class SubscriptionSchedulesPage extends StatefulWidget {
  ...
  /// Open the page at this date (defaults to today)
  final DateTime? initialDate;

  /// If provided, auto-scroll/highlight the card for this order
...
class _SubscriptionSchedulesPageState extends State<SubscriptionSchedulesPage>
  ...
  static const double _cardExtent = 240;
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (picked != null && picked != selectedDate) {
      setState(() {
        selectedDate = picked;
        _didScrollToFocus = false; // reset auto-scroll for new date
      });
      _fetchForDate(picked);
    }
  }
        ...
        body: RefreshIndicator(
          ...
          child: BlocBuilder<SubscriptionScheduleBloc, SubscriptionScheduleState>(
            builder: (context, state) {
              ...
              if (state is SubscriptionScheduleLoaded) {
                // Auto-scroll once to focused order
                if (!_didScrollToFocus && widget.focusOrderId != null) {
                  final idx =
                  state.schedules.indexWhere((s) => s.order.id == widget.focusOrderId);
                  if (idx >= 0) {
                    _didScrollToFocus = true;
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                      if (_scroll.hasClients) {
                        final target =
                        (idx * _cardExtent).clamp(0, _scroll.position.maxScrollExtent);
                        _scroll.animateTo(
                          target.toDouble(),
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        final isFocus = widget.focusOrderId != null &&
                            schedule.order.id == widget.focusOrderId;
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  _infoLine(theme,
                                      icon: Icons.home,
                                      label: schedule.customer.address.toString()),
                                  const Divider(),
                                  ...
                                  _infoLine(theme,
                                      icon: Icons.group,
                                      label: "Req’d Emps: ${schedule.order.employeeCount}"),
                                  _infoLine(theme,
                                      icon: Icons.confirmation_num,
                                      label: "Sched ID: ${schedule.scheduleId}"),
                                  _infoLine(theme,
                                      icon: Icons.schedule,
                                      label: "Time: ${schedule.scheduleTime}"),
                                  const Divider(),
                                    ...
                                    child: ElevatedButton.icon(
                                      ...
                                      onPressed: () async {
                                        ...
                                        final selected = await showDialog<Employee?>(
                                          context: context,
                                          builder: (_) =>
                                              _EmployeeDialog(employees: employees),
                                        );
                                        if (selected != null) {
                                          final already = schedule.assignedEmployees
                                              .any((e) => e.id == selected.id);
